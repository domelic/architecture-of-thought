...
The \texttt{/dcf} skill in this repository is itself an example of pattern capture. The underlying pattern---Socratic questioning applied to AI collaboration---was discovered through practice, recognized as reusable, and codified into a skill with 22 modes across six categories: workflow integration (\texttt{/dcf review}, \texttt{/dcf checkpoint}), debugging \& analysis (\texttt{/dcf debug}, \texttt{/dcf assumptions}), thinking \& learning (\texttt{/dcf learn}, \texttt{/dcf explain}), dialectic \& unblocking (\texttt{/dcf challenge}, \texttt{/dcf unstick}), and session management (\texttt{/dcf compact}, \texttt{/dcf context-health}). These modes can be chained into workflows for common scenarios---see Workflow Composition in the Glossary.
